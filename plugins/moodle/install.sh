#!/bin/bash
# Moodle plugin — runtime install hook.
#
...
#       moodle-mysql service over TCP.
#   openssh-client + sshpass — the agent ssh's into the moodle and
#       moodle-mysql containers as the clouve-ops operator account using
...
if [ "${#missing[@]}" -eq 0 ]; then
    echo "[moodle/install] runtime packages already present — skipping apt-get."
    exit 0
fi

echo "[moodle/install] installing missing packages: ${missing[*]}"

export DEBIAN_FRONTEND=noninteractive
if ! apt-get update -qq; then
    echo "[moodle/install] apt-get update failed — aborting" >&2
    exit 1
fi

if ! apt-get install -y --no-install-recommends "${missing[@]}"; then
    echo "[moodle/install] apt-get install failed for: ${missing[*]}" >&2
    exit 1
fi
...
echo "[moodle/install] runtime packages installed."
